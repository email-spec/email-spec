...
I want to verify that the example rails 3 app runs all of it's features as expected
  ...
  Scenario: generators test
    Given the rails3_root app is setup with the latest generators
    Then the rails3_root app should have the email steps in place

  Scenario: regression test
    Given the rails3_root app is setup with the latest email steps
    When I run "rake db:migrate RAILS_ENV=test" in the rails3_root app
    And I run "cucumber features -q --no-color" in the rails3_root app
